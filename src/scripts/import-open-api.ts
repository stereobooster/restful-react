...
const importOpenApi = async ({
  ...
  const haveGet = Boolean(output.match(/useGet</));
  const haveMutate = Boolean(output.match(/useMutate</));

  const imports = [];
  if (haveGet) {
    imports.push("useGet", "UseGetProps");
  }
  if (haveMutate) {
    imports.push("useMutate", "UseMutateProps");
  }
  output =
